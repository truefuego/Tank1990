...
public class brick {
    ...
    int brickHealth[] = {3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3};
    ...
    public brick()
    {
        breakBrickImage=new ImageIcon("Assets/break_brick.png");
        solidBrickImage=new ImageIcon("Assets/solid_brick.png");
    ...
    public boolean canMove(int x,int y,String dir) {
        ...
        for(int i=0; i< brickON.length;i++)
        {
            if(brickON[i]==1)
            {
                if(new Rectangle(x, y, dx, dy).intersects(new Rectangle(bricksXPos[i], bricksYPos[i], 50, 50)))
                {
                    flag = false;
                    break;
                }
        ...
        for(int i=0; i< solidBricksXPos.length;i++)
        {
            if(new Rectangle(x, y, dx, dy).intersects(new Rectangle(solidBricksXPos[i], solidBricksYPos[i], 50, 50)))
            {
                flag = false;
                break;
            }
